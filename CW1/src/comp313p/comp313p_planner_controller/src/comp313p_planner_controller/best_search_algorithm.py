...
class BestSearchAlgorithm(PlannerBase):
    ...
    def search(self, startCoords, goalCoords):
        ...
        while (self.isQueueEmpty() == False):
            ...
            # Find cell that brings us closest to the destination
            bestCell = cells[0]
            bestDistance = self.computeLStageAdditiveCost(bestCell, self.goal) 
            for nextCell in cells:
                if (self.hasCellBeenVisitedAlready(nextCell) == False):
                    if ( self.computeLStageAdditiveCost(nextCell, self.goal) < bestDistance):
                        bestCell = nextCell
                        bestDistance = self.computeLStageAdditiveCost(nextCell, self.goal)
                else:
                    self.resolveDuplicate(nextCell, cell)

            self.markCellAsVisitedAndRecordParent(bestCell, cell)
            self.pushCellOntoQueue(bestCell)
