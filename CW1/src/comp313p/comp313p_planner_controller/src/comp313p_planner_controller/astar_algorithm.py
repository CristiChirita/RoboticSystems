...
class AStarAlgorithm(PlannerBase):
	...
	self.heuristic = raw_input('Enter desired heuristic (any number, "C" cartesian, "O" octile, "M" manhattan): ')
	try: 
		isinstance(float(self.heuristic), float)
		self.alpha = 1
	except ValueError:
		self.alpha = float(raw_input('Enter desired weighting: '))
    ...
    def computeManhattanCost(self, parentCell, cell):
    ...
    def computeOctilleCost(self, parentCell, cell):
        ...
        dX = abs(cell.coords[0] - parentCell.coords[0])
        dY = abs(cell.coords[1] - parentCell.coords[1])
    ...
    def computeCost(self, parentCell, cell):
        ...
        try:
                isinstance(float(self.heuristic), float)
		return L + self.alpha * float(self.heuristic)
	except ValueError:
            if self.heuristic == "C":
