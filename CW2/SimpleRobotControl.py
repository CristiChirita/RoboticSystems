		...
		while not rospy.is_shutdown():
			...
			try:
				...
				if targetX - position.x > 0: #target to the right
					if theta != 0: 
						if theta >= 180:
							setAngle(5, theta, True)
						else:
							setAngle(5, theta - 180, False)
					move(1, targetX -  position.x)
				else: #target to the left
					if theta != 180:
						if theta >= 180:
							setAngle(5, 180 - theta, False)
						else:
							setAngle(5, theta - 180, True)
					move(1, position.x - targetX)
				...
				if targetY - position.y > 0: #target higher
					if theta != 90:
						if (theta < 90):
							setAngle(5, 90 - theta, False)
						elif theta > 270:
							setAngle(5, 360 - theta + 90, False)
						else:
							setAngle(5, theta - 90, True)
					move(1, targetY - position.y)
				else: #target lower
					if theta != 270:
						if theta < 90:
							setAngle(5, theta + 90, True)
						elif theta > 270:
							setAngle(5, theta - 270, True)
						else:
							setAngle(5, 270 - theta, False)
					move(1, position.y - targetY)
				
				#Adjust to specified angle
				theta = self.getTheta()
				if targetAngle > theta:
					setAngle(5, targetAngle - theta, False)
				else:
					setAngle(5, theta - targetAngle, True)
